# Slicing lokal dan HPP

QR Plate, Keychain, dan Clicker memiliki tombol **Slice & hitung HPP**. API Numa3D menjalankan OrcaSlicer pada komputer tempat server Numa3D berjalan, membaca statistik G-code, lalu menghitung HPP per plate. Perubahan asumsi biaya langsung menghitung ulang HPP tanpa slicing ulang.
...
- `costs`: JSON opsional, memakai nama properti `SLICER_COST_DEFAULTS` dalam `utils/slicerHpp.js`.

Respons memuat `totalGrams`, `printTimeSeconds`, `filamentGrams`, `colors`, `filamentChanges`, `primeTower`, `slicerVersion`, `profile`, serta `hpp` (total, rincian, dan asumsi). Total gram memakai angka total dari Orca, karena penjumlahan slot yang masing-masing dibulatkan bisa berbeda 0,01 g. Durasi mesin dihitung sekali untuk seluruh plate.

Server memanggil [CLI slicing OrcaSlicer](https://www.orcaslicer.com/wiki/cli/cli_actions); ini bukan HTTP API bawaan aplikasi desktop. Konfigurasi Windows default:
...
Variabel bersifat opsional pada instalasi Windows di lokasi standar. Pada server lain, OrcaSlicer dan profil Anycubic harus tersedia di server tersebut; deployment cloud tidak otomatis mengakses OrcaSlicer di PC pengguna.
...
- `node --test tests/slicerHpp.test.js tests/slicerProfiles.test.js tests/hpp.test.js`: 13 tes lulus.
- Slicing nyata ketiga sampel melalui `sliceGenerator3mf` berhasil.
- Uji HTTP dengan handler dan middleware auth asli: request tanpa sesi mendapat 401; request multipart terautentikasi menghasilkan 4,65 g dan HPP Rp4.355 untuk sampel Keychain AB.
- Statistik, perhitungan biaya, cache profil, validasi input, dan pemisahan waktu mesin diuji. Cetak fisik belum dilakukan.
