...
test('3MF separates stand from multipart plaque, STL keeps all parts and SCAD includes stand and icon', async () => {
  ...
  try {
    ...
    assert.match(settings.print_settings_id, /QR Plate Detail/)
    const plainZip = unzipSync(new Uint8Array(await result.get3mfBlob({ processPreset: null }).arrayBuffer()))
    ...
    const restoredZip = unzipSync(new Uint8Array(await result.get3mfBlob().arrayBuffer()))
    assert.deepEqual(restoredZip['Metadata/project_settings.config'], zip['Metadata/project_settings.config'])
