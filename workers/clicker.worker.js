import { generateClickerCore } from '../utils/clickerCore.js'
...
function collectTransferables(result) {
  ...
  for (const part of [
    ...
    ...(result.lidPreviewParts || result.accentPreviewParts || []),
  ...
  add(result.lidStlBuffer || result.accentStlBuffer)
...
self.onmessage = async (event) => {
  ...
  try {
    const result = await generateClickerCore(opts)
